...
def render():
    ...
    with col2:
        if not inputs["rittal"]:
            st.success("CUBIC BOM: OK") if not miss_B else st.warning(f"Project BOM missing: {miss_B}")
        else:
    ...
    # Instructions → SDD07550 + papildomi komponentai
    if df_instr is not None and not df_instr.empty:
