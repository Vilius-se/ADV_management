...
ready = all([file_cubic, file_bom, file_data, file_ks, doc_number])
...
if ready:
    try:
        ...
        _part_b2a=dict(zip(df_part_no['Norm_B'], df_part_no['PartNo_A']))
        ...
        st.success("Failai įkelti, galite suvesti mechanikos kiekius")

        st.subheader("🛠️ Mechanikos kiekių įvedimas")
        if 'mech_vals' not in st.session_state or len(st.session_state['mech_vals']) != len(df_filtered):
            st.session_state['mech_vals'] = [0]*len(df_filtered)

        with st.form("mech_form"):
            for i, row in df_filtered.iterrows():
                st.session_state['mech_vals'][i] = st.number_input(
                    f"{row['Vendor']} — {row['Description']} [Max {int(row['Quantity'])}]",
                    min_value=0,
                    max_value=int(row['Quantity']),
                    step=1,
                    value=int(st.session_state['mech_vals'][i]),
                    key=f"mech_{i}"
                )
            saved = st.form_submit_button("💾 Išsaugoti mechanikos kiekius")

        if saved:
            st.success("Mechanikos kiekiai išsaugoti. Dabar galite generuoti.")

        gen = st.button("✅ Generuoti Excel")
        if gen:
            mech_vals = st.session_state.get('mech_vals', [0]*len(df_filtered))

            mech_entries = []
            for (i, r), q in zip(df_filtered.iterrows(), mech_vals):
                if q and q>0:
                    mech_entries.append({
                        'Type':r['Type'],
            ...
            df_remaining['Kiekis įvestas']=mech_vals
            ...
            if 'mech_alloc' in locals() and not mech_alloc.empty:
                st.subheader("📦 Mechanika"); st.dataframe(mech_alloc, use_container_width=True)
            if 'prod_alloc' in locals() and not prod_alloc.empty:
                st.subheader("🏭 Gamyba"); st.dataframe(prod_alloc, use_container_width=True)
            if 'df_used_total' in locals() and not df_used_total.empty:
                st.subheader("📉 Paimta iš Stock"); st.dataframe(df_used_total, use_container_width=True)
            if 'sms_alloc' in locals() and not sms_alloc.empty:
                st.subheader("🔌 BOM Schneider Main Switch"); st.dataframe(sms_alloc, use_container_width=True)
            if 'other_alloc' in locals() and not other_alloc.empty:
                st.subheader("⚙️ BOM Likę Komponentai"); st.dataframe(other_alloc, use_container_width=True)

            with pd.ExcelWriter(export_filename,engine="openpyxl") as writer:
                if 'mech_alloc' in locals() and not mech_alloc.empty: mech_alloc.to_excel(writer,sheet_name="Mechanika",index=False)
                if 'prod_alloc' in locals() and not prod_alloc.empty: prod_alloc.to_excel(writer,sheet_name="Gamyba",index=False)
                if 'df_used_total' in locals() and not df_used_total.empty: df_used_total.to_excel(writer,sheet_name="Paimta iš Stock",index=False)
                if 'sms_alloc' in locals() and not sms_alloc.empty: sms_alloc.to_excel(writer,sheet_name="BOM Schneider Main Switch",index=False)
                if 'other_alloc' in locals() and not other_alloc.empty: other_alloc.to_excel(writer,sheet_name="BOM Likę Komponentai",index=False)
